<template>
  <main class="flex flex-col min-h-screen bg-slate-900 text-slate-100 font-sans">
    ...
    <div class="flex-1 w-full max-w-7xl mx-auto p-6">
      ...
      <Transition name="fade-slide" mode="out-in">
        ...
        <div v-else key="detail" class="w-full max-w-4xl mx-auto flex flex-col items-center">
          ...
          <div class="w-full bg-slate-800/80 backdrop-blur-xl p-8 md:p-14 rounded-[2.5rem] shadow-2xl border border-slate-700 relative overflow-hidden flex flex-col items-center text-center">
            ...
            <img 
              ...
              class="w-48 sm:w-64 rounded-xl shadow-2xl shadow-black/60 border-2 border-slate-600 mb-8 relative z-10"
              alt="National Flag"
            >
            
            <h2 class="text-4xl md:text-5xl font-black mb-6 text-white tracking-tight drop-shadow-md relative z-10">
              {{ selectedCountry.name }}
            </h2>
            
            <div class="flex flex-wrap justify-center gap-3 sm:gap-6 mb-8 relative z-10">
               <span class="px-5 py-2 bg-slate-900/80 border border-slate-700 rounded-xl text-slate-300 text-sm font-bold tracking-wider flex items-center gap-2 shadow-inner">
                 <span class="text-lg">🏛️</span> {{ selectedCountry.capital || 'N/A' }}
               </span>
               <span class="px-5 py-2 bg-blue-500/10 border border-blue-500/30 text-blue-400 rounded-xl text-sm font-bold tracking-widest uppercase shadow-inner">
                 {{ selectedCountry.continent }}
               </span>
            </div>
            
            <div class="w-48 h-48 sm:w-56 sm:h-56 bg-slate-900/50 rounded-full flex items-center justify-center p-8 border border-slate-700/50 mb-10 shadow-inner relative z-10 transition-transform duration-700 hover:scale-105">
              <img 
                :src="`/all/${selectedCountry.code.toLowerCase()}/vector.svg`" 
            ...
            <div class="flex flex-col sm:flex-row gap-4 w-full max-w-md relative z-10">
              <button @click="nextCountry" class="flex-1 px-6 py-5 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-2xl shadow-lg shadow-blue-600/30 transition-all transform hover:-translate-y-1 uppercase tracking-widest text-sm flex items-center justify-center gap-2">
                Next <span class="text-lg leading-none">➔</span>
              </button>
              
              <button @click="randomCountry" class="flex-1 px-6 py-5 bg-slate-700 hover:bg-slate-600 text-white border border-slate-600 hover:border-slate-500 font-black rounded-2xl shadow-lg transition-all transform hover:-translate-y-1 uppercase tracking-widest text-sm flex items-center justify-center gap-2">
